...
6. **חגים/ימי מסחר מקוצרים** — הבריף לא התייחס לכך שהבורסה סגורה בחגים אמריקאיים,
   ושבימי מסחר מקוצרים NQ ממשיך להיסחר (לא "קופא"). **נפתר** — בדיקת
   `get_trading_day_type` מול לוח `CME_Equity` (לא NYSE cash) לפני כל job מסחרי,
   שמבחינה בין `'regular'`/`'early_close'`/`'holiday'`; ימי `'holiday'` מסומנים
   `SKIPPED` ולא נכנסים למכנה הסטטיסטי, וימי `'early_close'` נבדקים מול שעת הסגירה
   הרשמית בפועל של אותו יום (ראו סעיף 6 המפורט).
